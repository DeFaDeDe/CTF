...
After we launch the machine, we will see a corridor, with many doors

![image.png](images/image%201.png)

The doors are interactable, and viewing their source, we can find all the endpoints
...
Click on any of the doors will redirect us to a white room

![image.png](images/image%202.png)

We can try to extract every endpoint.
...
They seem random, but are they hashed or encrypted? We can try to check hashes on [hashes.com](https://hashes.com/en/decrypt/hash) 

![image.png](images/image%203.png)

So it is actually representing 1, so it is just some hashed indices

Since there are 13 endpoints available, meaning that `c51ce410c124a10e0db5e4b97fc2af39` should be 13. Do a quick check, and we can find that it is true

However, we also need to know which algorithm it uses. We can use [the hash identifier]([https://hashes.com/en/tools/hash_identifier](https://hashes.com/en/tools/hash_identifier)) to check

![image.png](images/image%204.png)

MD5 Hashes, so all we need to do is try to generate the hashes of 0 and 14, and see which one works

To do that, we can write a Python script to generate the URL for us.

```bash
import hashlib
num='0'
result=hashlib.md5(num.encode())
print(f'0: http://<Target IP>/{result.hexdigest()}')
num2='14'
result2=hashlib.md5(num2.encode())
print(f'14: http://<Target IP>/{result2.hexdigest()}')
```

Running the script will give us the URLs to test

```bash
└─$ python solve.py
0: http://<Target IP>/cfcd208495d565ef66e7dff9f98764da
14: http://<Target IP>/aab3238922bcc25a6f606eb525ffdc56
```

The first one (0), which is the `/cfcd208495d565ef66e7dff9f98764da` endpoint, shows us the flag.
